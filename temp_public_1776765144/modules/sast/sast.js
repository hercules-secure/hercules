...
class GitRepositoryHandler {
    ...
    async checkRepositoryExists(url) {
        ...
        try {
            if (type === 'github') {
                ...
                if (response.status === 200) return { exists: true, type, owner, repo };
                if (response.status === 404) return { exists: false, error: `Проект ${owner}/${repo} не найден на GitLab` };
                return { exists: false, error: `GitLab API вернул статус ${response.status}` };
            }
            else {
                try {
                    await execAsync(`git ls-remote ${url} HEAD`, { timeout: 10000 });
                    return { exists: true, type, owner, repo };
                } catch (error) {
    ...
    parseRepositoryUrl(url) {
        ...
        if (cleanUrl.startsWith('http')) {
            const match = cleanUrl.match(/https?:\/\/(?:www\.)?(?:github\.com|gitlab\.com|bitbucket\.org)\/([^\/]+)\/([^\/]+)/);
            if (match) { owner = match[1]; repo = match[2]; }
        } else if (cleanUrl.startsWith('git@')) {
            const match = cleanUrl.match(/git@(?:github\.com|gitlab\.com|bitbucket\.org):([^\/]+)\/([^\/]+)/);
            if (match) { owner = match[1]; repo = match[2]; }
        }
...
class AnalysisEngine {
  ...
  isRuleApplicableForFile(rule, filePath) {
    ...
    // Go
    if (rule.languages.includes('go') && ext === '.go') {
      return true;
    }
    
    // Python
    if (rule.languages.includes('python') && ext === '.py') {
      return true;
    }
    
    // JavaScript/TypeScript
    if ((rule.languages.includes('javascript') || rule.languages.includes('typescript')) && 
        (ext === '.js' || ext === '.ts' || ext === '.jsx' || ext === '.tsx')) {
      return true;
    }
    
    // Java
    if (rule.languages.includes('java') && ext === '.java') {
      return true;
    }
    
    // Config
    if (rule.languages.includes('config') || rule.languages.includes('env') || rule.languages.includes('json')) {
      if (ext === '.json' || fileName === '.env' || ext === '.conf' || ext === '.config') {
        return true;
      }
    }
      ...
      for (const doc of docs) {
        ...
        for (let i = 0; i < containers.length; i++) {
          ...
          if (container.securityContext?.privileged === true) {
            ...
            if (rule) {
              this.addResult(rule, filePath, 0, 0, 'securityContext.privileged: true');
            }
          }
          ...
          if (!container.resources || (!container.resources.limits && !container.resources.requests)) {
            ...
            if (rule) {
              this.addResult(rule, filePath, 0, 0, 'resources: not set');
            }
          }
        }
        ...
        if (podSpec.securityContext?.runAsNonRoot === false || podSpec.securityContext?.runAsUser === 0) {
          ...
          if (rule) {
            this.addResult(rule, filePath, 0, 0, 'securityContext.runAsNonRoot: false or runAsUser: 0');
          }
        }
        ...
        for (let i = 0; i < volumes.length; i++) {
          if (volumes[i].hostPath) {
            ...
            if (rule) {
              this.addResult(rule, filePath, 0, 0, `hostPath: ${volumes[i].hostPath.path}`);
            }
          }
        }
      }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const extensionsToAnalyze = ['.yaml', '.yml', '.tf', '.tfvars', '.hcl', '.json', '.env', 
            '.py', '.go', '.js', '.ts', '.jsx', '.tsx', '.java', '.xml', '.conf', '.config'];
          
          const specialFiles = ['dockerfile', 'containerfile', 'docker-compose.yml', 'values.yaml', 'chart.yaml'];
          
          const isExtensionMatch = extensionsToAnalyze.includes(ext);
          const isSpecialFile = specialFiles.includes(fileName) || fileName === 'Dockerfile';
  ...
  async analyzeFile(filePath) {
    try {
      ...
      if (ext === '.yaml' || ext === '.yml' || ext === '.tf' || ext === '.tfvars' || ext === '.hcl' ||
          path.basename(filePath).toLowerCase() === 'dockerfile') {
