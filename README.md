...
## BLENDER (Универсальный анализатор безопасности кода)

Модуль для комплексного анализа безопасности кода, объединяющий SCA, SAST и Fuzzing в едином инструменте.

### Ключевые возможности

| Категория | Возможности |
|-----------|-------------|
| `Анализ зависимостей` | `Формирование SBOM (CycloneDX)`, `Поиск уязвимостей (OSV/NVD)`, `Проверка лицензий` |
| `Анализ кода` | `Поиск секретов в коде`, `Выявление уязвимых паттернов`, `Проверка качества кода` |
| `API тестирование` | `Анализ API эндпоинтов`, `Фаззинг API`, `Автоматическая генерация тестов` |
| `Интеграция` | `CI/CD интеграция`, `GitHub Actions`, `GitLab CI`, `Jenkins` |
...
| `C/C++` | `CMake`, `Conan`, `vcpkg` | `CMakeLists.txt`, `conanfile.txt`, `vcpkg.json` |

### Форматы API спецификаций
...
| Источник | Описание |
|----------|----------|
| `GitHub` | `Публичные и приватные репозитории` |
| `GitLab` | `Публичные и приватные репозитории` |
| `Bitbucket` | `Публичные и приватные репозитории` |
| `Архив` | `ZIP, TAR, GZ, 7Z` |
| `Локальный проект` | `Локальная файловая система` |
| `Корпоративные репозитории` | `Nexus, Artifactory, GitLab Self-Hosted` |

### Форматы отчетов

| Формат | Назначение |
|--------|------------|
| `HTML` | `Визуальный отчет для человека` |
| `JSON` | `Структурированные данные для автоматизации` |
| `PDF` | `Печатная версия отчета` |
