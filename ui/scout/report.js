// report.js - полная версия с вкладками Безопасность, Структура, Проблемы
...
function generateScoutFullHTMLReport(result) {
    ...
    // ИСПРАВЛЕННАЯ группировка - API проверяются первыми
    ...
    const subdomainsTable = () => {
        if (groups.subdomains.size === 0 && (!subdomainsData.subdomains || subdomainsData.subdomains.length === 0)) {
        ...
        if (subdomainsData.subdomains && subdomainsData.subdomains.length > 0) {
            rows += subdomainsData.subdomains.map((s, idx) => `
                <tr>
                    ...
                    <td>${s.ip || '—'}</td>
                    <td><span class="status-badge status-active">обнаружен</span></td>
                </tr>
            `).join('');
        }
        ...
        for (const [subdomain, items] of groups.subdomains) {
            rows += `
                <tr>
                    ...
                    <td>${escapeHtml(subdomain)}</td>
                    <td>—</td>
                    <td><span class="status-badge status-active">найдено ${items.length} путей</span></td>
                </tr>
            `;
        }
        ...
        return `
            ...
            <div class="table-footer">Всего субдоменов: ${subdomainsData.subdomains?.length || 0}</div>
        `;
    };
    ...
    const apiTable = () => {
        ...
        return `
            <table class="data-table sortable" id="apiTable">
                <thead>
                    <td><th style="width:50px">#</th><th>API путь</th><th style="width:80px">HTTP</th><th style="width:100px">Риск</th><th>Описание</th></tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
        `;
    ...
    const filesTable = () => {
        ...
        const rows = groups.files.map((item, idx) => {
            ...
            return `
                <tr data-risk="${risk}">
                    <td class="index-cell">${idx + 1}</td>
                    <td><code>${escapeHtml(fileName)}</code><br><small style="color:#999;">${escapeHtml(item.path)}</small></td>
                    <td style="text-align:center">${item.status || '?'}</td>
                    <td><span class="risk-badge" style="background:${riskColors[risk]}">${riskNames[risk]}</span></td>
                    <td class="truncate">${escapeHtml(item.recommendation || '—')}</td>
                </tr>
    ...
    const allPathsTable = () => {
        ...
        return `
            <table class="data-table sortable" id="allPathsTable">
                <thead>
                    <td><th style="width:50px">#</th><th>Путь</th><th style="width:80px">HTTP</th><th style="width:100px">Риск</th><th>Описание</th></tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
        `;
    ...
    const securityTable = () => {
        ...
        if (ports.issues && ports.issues.length > 0) {
            ...
            html += `
                <table class="data-table">
                    <thead><tr><th style="width:50px">#</th><th>Порт</th><th style="width:100px">Риск</th></table></thead>
                    <tbody>
...
<body>
    <div class="report-container">
        ...
        <div class="tabs" id="tabs">
            ...
            <button class="tab-btn" data-tab="subdomains">Субдомены <span class="tab-count">${groups.subdomains.size + (subdomainsData.subdomains?.length || 0)}</span></button>
        ...
        <div class="report-footer">
            ...
            <p>Всего обнаружено: ${accessiblePaths.length} путей | API вызовов в JS: ${apiFromJS.apiEndpoints?.length || 0}</p>
        </div>
    </div>
...
// Функция для отображения отчета в модальном окне
// Функция для отображения отчета в модальном окне (парсит JSON и рендерит отдельно)
function showScoutReportModal(result, onClose) {
    ...
    const allIssuesTable = () => {
        ...
        if (!links.brokenLinks || links.brokenLinks.length === 0) {
            ...
            html += `
                <table class="data-table">
                    <thead><td><th style="width:50px">#</th><th>URL</th><th style="width:80px">Статус</th></tr></thead>
                    <tbody>
                        ${links.brokenLinks.slice(0, 50).map((link, idx) => `
                            <tr>
                                <td class="index-cell">${idx + 1}</td>
