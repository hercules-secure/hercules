export function generateScoutReportHTML(result) {
    ...
    function generateOsintHTML(osintData) {
        ...
        if (emails.length === 0) {
            html += `<div class="empty-state">Email адреса не обнаружены</div>`;
        } else {
            html += `
                <table class="data-table">
                    <thead>
                        <tr><th style="width:50px">#</th><th>Email</th><th>Домен</th><th>Источник</th></tr>
                    </thead>
                    <tbody>
                        ${emails.map((item, idx) => `
                            <tr>
                ...
                <div class="table-footer">Всего email: ${emails.length}</div>
            `;
        }
        html += `</div>`;
        ...
        if (phones.length === 0) {
            html += `<div class="empty-state">Телефоны не обнаружены</div>`;
        } else {
            html += `
                <table class="data-table">
                    <thead>
                        <tr><th style="width:50px">#</th><th>Телефон</th><th>Оригинал</th><th>Источник</th></tr>
                    </thead>
                    <tbody>
                        ${phones.map((item, idx) => `
                            <tr>
                                <td class="index-cell">${idx + 1}</td>
                                <td><code style="color:#e83e8c;">${escapeHtml(item.phone)}</code></td>
                                <td style="font-size:11px; color:#666;">${escapeHtml(item.original || '—')}</td>
                                <td style="font-size:11px; color:#666;">${escapeHtml(item.source || '—')}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                <div class="table-footer">Всего телефонов: ${phones.length}</div>
            `;
        }
        html += `</div>`;
        ...
        if (socials.length === 0) {
            html += `<div class="empty-state">Социальные сети не обнаружены</div>`;
        } else {
            html += `
                <table class="data-table">
                    <thead>
                        <tr><th style="width:50px">#</th><th>Платформа</th><th>URL</th><th>Username</th></tr>
                    </thead>
                    <tbody>
                        ${socials.map((item, idx) => `
                            <tr>
                                <td class="index-cell">${idx + 1}</td>
                                <td><span style="background:#e9ecef; padding:2px 10px; border-radius:12px; font-size:11px;">${escapeHtml(item.platform)}</span></td>
                                <td><a href="${escapeHtml(item.url)}" target="_blank" style="color:#667eea; font-size:11px;">${escapeHtml(item.url.length > 60 ? item.url.substring(0, 60) + '...' : item.url)}</a></td>
                                <td style="font-size:11px; color:#666;">${escapeHtml(item.username || '—')}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                <div class="table-footer">Всего соцсетей: ${socials.length}</div>
            `;
        }
        html += `</div>`;
        
        // ============================================================
        // ДОПОЛНИТЕЛЬНАЯ ИНФОРМАЦИЯ - ТАБЛИЦА (ИЗМЕНЕНО)
        // ============================================================
    ...
    const subdomainsTable = () => {
        ...
        const rows = subdomainsList.map((s, idx) => {
            const statusColor = s.status === '200' ? '#28a745' : (s.status === '302' ? '#ffc107' : (s.status === '401' ? '#fd7e14' : (s.status === '403' ? '#dc3545' : '#6c757d')));
            const statusLabel = s.status || 'unknown';
            
            return `
                <tr>
                    <td class="index-cell">${idx + 1}</td>
                    <td class="domain-cell"><a href="${s.url || 'https://' + s.domain}" target="_blank">${escapeHtml(s.domain)}</a></td>
    ...
    const ipsTable = () => {
        ...
        const rows = ipsList.map((item, idx) => {
            const statusColor = item.status === '200' ? '#28a745' : (item.status === '302' ? '#ffc107' : (item.status === '401' ? '#fd7e14' : (item.status === '403' ? '#dc3545' : '#6c757d')));
            
            return `
                <tr>
                    <td class="index-cell">${idx + 1}</td>
                    <td class="ip-cell"><code>${escapeHtml(item.ip)}</code></td>
                    <td>${escapeHtml(item.domain || '—')}</td>
                    ...
                    <td class="source-cell">${escapeHtml(item.source || '—')}</td>
                </tr>
            `;
        }).join('');
    ...
    const filesTable = () => {
        ...
        const rows = groups.files.map((item, idx) => {
            ...
            return `
                <tr data-risk="${risk}">
                    <td class="index-cell">${idx + 1}</td>
                    <td class="path-cell"><code>${escapeHtml(fileName)}</code></td>
                    <td class="status-cell">${item.status || '?'}</td>
                    <td class="risk-cell risk-${risk}">${riskLabel}</td>
                    <td class="desc-cell">${escapeHtml(item.recommendation || '—')}</td>
                </tr>
    ...
    <div class="report-container">
        ...
        <div class="content">
            ...
            <div class="tabs" id="tabs">
                ...
                <button class="tab-btn" data-tab="users">Пользователи <span class="tab-count">${usersCount}</span></button>
            </div>
        ...
        <div class="report-footer">
            <p>Сгенерировано с помощью Геркулес | Скаут</p>
            <p>Инструмент разведки веб-приложений</p>
        </div>
    </div>
