...
// Настройка multer ДОЛЖНА быть здесь
...
async function extractArchive(archivePath, originalName, extractDir) {
    ...
    if (isZip) {
        ...
        throw new Error(`Неподдерживаемый формат архива: ${fileName}`);
    }
}
...
export function createSCARouter(options = {}) {
    ...
    // POST /upload - загрузка архива (должен быть ПЕРВЫМ)
    ...
    // POST / - анализ репозитория по URL
