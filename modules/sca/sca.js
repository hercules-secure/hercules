...
class BaseSBOMAnalyzer {
    constructor(repoUrl, options = {}) {
        ...
        // Кэш для содержимого директорий
        this.cache = new Map();
    ...
    async generateSBOM() {
        try {
            ...
            const libraries = allComponents.filter(c => 
                c && !c.properties?.some(p => p.name === 'src:type' && (p.value === 'root' || p.value === 'package'))
            );
            ...
            const sbom = {
                ...
                metadata: {
                    ...
                    tools: [
                        {
                            ...
                            properties: [
                                ...
                                },
                                {
                                    ...
                                    value: "vulnerability-scanning, dependency-analysis"
                                }
                            ]
                        }
                    ],
                    component: {
                        type: "application",
                        name: (this.repoInfo && this.repoInfo.repo) ? this.repoInfo.repo : 'unknown',
                        version: "1.0.0",
                        purl: `pkg:generic/${(this.repoInfo && this.repoInfo.repo) ? this.repoInfo.repo : 'unknown'}@1.0.0`,
                        properties: [
                            {
                                name: "repository:url",
                                value: this.repoUrl
                            },
                            {
                                ...
                                value: (this.repoInfo && this.repoInfo.owner) ? this.repoInfo.owner : 'unknown'
                            },
                            {
                                name: "scan:startTime",
                                value: this.scanMetadata.startTime
                            }
                        ]
                    },
                    properties: []
            ...
            const metadataProps = [
                ...
                {
                    name: "scan:summary:totalLibraries",
                    value: String(libraries.length)
                },
                {
                    name: "scan:summary:totalRootComponents",
                    value: String(rootComponents.length)
                },
                {
                    name: "scan:summary:totalDependencies",
                    value: String(dependencies.length)
                },
                {
                    name: "scan:summary:totalVulnerabilities",
                    value: String(vulnerabilityStats.total)
                },
                {
                    name: "scan:summary:vulnerabilities:critical",
                    value: String(vulnerabilityStats.critical)
                },
                {
                    name: "scan:summary:vulnerabilities:high",
                    value: String(vulnerabilityStats.high)
                },
                {
                    name: "scan:summary:vulnerabilities:medium",
                    value: String(vulnerabilityStats.medium)
                },
                {
                    name: "scan:summary:vulnerabilities:low",
                    value: String(vulnerabilityStats.low)
                },
                {
                    name: "scan:summary:vulnerabilities:unknown",
                    value: String(vulnerabilityStats.unknown)
                }
            ];
                    ...
                    affects: [
                        {
                            ...
                            versions: [
                                {
                                    version: v.vulnerability.affects?.[0]?.versions?.[0] || "unknown"
                                }
                            ]
                        }
                    ],
                    ...
                    properties: [
                        {
                            ...
                            value: v.vulnerability.componentName
                        },
                        {
                            name: "vulnerability:componentVersion",
                            value: v.vulnerability.componentVersion
                        },
                        {
                            name: "vulnerability:severity",
                            value: v.vulnerability.severity || "UNKNOWN"
                        }
                    ]
                }));
            }
            ...
            if (recommendations && recommendations.length > 0) {
                ...
                const subjects = vulnerabilities && Array.isArray(vulnerabilities) 
                    ? vulnerabilities.map(v => v && v.bomRef).filter(Boolean)
                    : [];
                
                recommendations.forEach((rec, index) => {
                    sbom.annotations.push({
                        "bom-ref": `recommendation-${index + 1}`,
                        subjects: subjects,
                        text: rec,
                        timestamp: new Date().toISOString()
                    });
